...
> Полный технический документ проекта. Версия 2.0.
...
### Концепция
**Dark Fantasy RPG** — референс Path of Exile 2, Diablo IV UI, Elden Ring меню. Не просто тёмная тема с золотым акцентом. Это переосмысление навигации в духе игрового интерфейса: геральдика, угловые орнаменты, рунические символы.

Персонаж на сайдбаре — SVG-геометрия (голова, тело, броня, оружие), строится из прямоугольников и полигонов. Меняется при прокачке: новая броня, плащ, шлем, оружие, аура. Никаких внешних изображений — весь дизайн генерируется кодом.

### Цветовая палитра
```
Фон основной:      #07080a   — почти чёрный с холодным синим оттенком
Фон поверхности:   #0d0e12
Фон карточек:      #12141a
Фон панелей:       #181c24
Золото (акцент):   #c8a96e   — XP, рамки, активные элементы
Золото яркое:      #e8c87a   — имена, заголовки
Золото dim:        #4a3c28   — неактивные бордеры
Текст яркий:       #e8dcc8
Текст средний:     #9a8c76
Текст тёмный:      #3a3428
```

Цвета секций:
```
Алгоритмы:     #1a3a6a / #6a9fd4  (синий)
SQL:           #0d2808 / #639922  (зелёный)
Go / Backend:  #2a1800 / #EF9F27  (янтарный)
System Design: #1a1040 / #7F77DD  (фиолетовый)
Behavioral:    #04180f / #1D9E75  (бирюзовый)
```

Сложность подземелий:
```
Normal:  #639922  зелёный
Hard:    #EF9F27  золотой
Boss:    #c0392b  красный
```

### Типографика
- **Заголовки и навигация:** Cinzel / Cinzel Decorative (Google Fonts)
- **UI и тело:** Inter / system-ui
- **Код:** JetBrains Mono

### Компоненты
- Бордеры: 1px solid `#4a3c28`, без скруглений
- Угловые акценты: L-образные золотые линии в углах карточек
- Разделители: `✦ НАЗВАНИЕ ✦` с горизонтальными линиями
- Кнопки: прямые углы или один срезанный угол
- Иконки: рунические символы `◈ ⚜ ⚗ ⚔ ✦ ⊕`
- Никаких градиентов в UI

### Навигация
```
Топбар: DRUZ9 | Season II · The Recursion | Sanctum Arena Guild Atlas Codex | XP Lvl24 Аватар
```

Игровые названия разделов (не переводятся ни на каком языке):
- **Sanctum** — главное лобби / хаб
- **Arena** — PvP режимы
...
### Layout: три колонки
```
[Сайдбар персонажа 220px] [Основной контент flex] [Правый сайдбар 180px]
```
...
Стартовая страница после логина. Центр прогрессии.

**Блоки:**
- Портрет персонажа + класс + уровень + XP
- Weekly AI Report (последние 7 дней)
- Мини-превью Skill Atlas с выделением слабых зон
- Карьерная линия: где ты на пути
- История активности: арена, мок, гильдия
- Solo + Guild рейтинг
- Сезонный прогресс
- AI-план на следующую неделю
...
| Фича | Описание |
|---|---|
| Replay сессии | Таймлайн набора кода — перемотай и увидь себя со стороны |
| Стресс-метрика | Паузы, откаты, хаос → стресс-график. "Ты нервничаешь на System Design" |
| Психологический профиль | AI находит паттерны под давлением |
| Компания = Dungeon | Карта с комнатами-секциями, финальный Boss |
| Карьерная линия | Junior → Principal вместо абстрактного числа |
| Порча навыков | Не практикуешься — теряешь прогресс |
| Парный AI-мок | Двое проходят мок, AI — интервьюер для обоих |
| AI-адвокат дьявола | Режим провокаций и давления |
| Некромантия | Найди баг в чужом анонимном решении → XP |
| Hardcore режим | Провал в арене = потеря прогресса |
| Проклятые задачи | Дебафф + тройной XP |
| Spectator mode | Смотри live как другой проходит мок |
| Prestige | Сброс с постоянным бонусом и редким классом |
| Стратегия первых 100 | AI-бот как соперник при малой аудитории |
...
### Промпт для дизайна (Claude / Figma AI)

```
Ты опытный product designer. Разработай полную дизайн-систему и ключевые
экраны для веб-платформы druz9 (Друзья) — подготовки к техническим
собеседованиям в стиле Dark Fantasy RPG Path of Exile 2.

ВИЗУАЛЬНЫЙ СТИЛЬ:
Референс: Path of Exile 2, Diablo IV UI, Elden Ring меню.
Полноценный игровой интерфейс: геральдика, угловые орнаменты, Cinzel шрифт.
Персонаж — SVG-геометрия (прямоугольники + полигоны), меняется при прокачке.

ЦВЕТА (строго):
#07080a фон, #12141a карточки, #c8a96e золото, #e8c87a золото яркое,
#4a3c28 gold dim, #e8dcc8 текст яркий, #9a8c76 текст средний
Секции: #6a9fd4 алго, #639922 SQL, #EF9F27 Go, #7F77DD SD, #1D9E75 BEH

ШРИФТЫ: Cinzel Decorative (заголовки), Inter (UI), JetBrains Mono (код)

ДЕТАЛИ:
- Бордеры 1px #4a3c28, прямые углы, L-образные угловые акценты
- Разделители: ✦ НАЗВАНИЕ ✦ + горизонтальные линии
- Иконки: ◈ ⚜ ⚗ ⚔ ✦ ⊕  |  Никаких градиентов

ЭКРАНЫ:

1. SANCTUM (главное лобби) — три колонки:
Левый 220px: SVG персонаж, класс "Backend Architect", 4 атрибута со шкалами,
меню (Практика: AI-мок/Live-мок/Редактор | Испытания: Арена/2v2/Гильдии)
Центр: сетка компаний-подземелий с бейджами Normal/Hard/Boss и угловыми акцентами,
прогресс-бары, лог активности с иконками и XP
Правый 180px: 4 флакона силы, топ-5 рейтинга, сезонный трек с ромбами
Топбар: DRUZ9 | Season II · The Recursion | Sanctum Arena Guild Atlas Codex | XP Lvl аватар

2. AI-МОК СЕССИЯ:
60% Monaco Editor (#0a0c10, Go синтаксис, gold нумерация строк)
Шапка: хлебные крошки + таймер в gold рамке
5 секций-пилюль вверху: done=зелёные, active=фиолетовая, locked=серые
40% панель AI: сообщения (серые=вопрос, янтарные=предупреждение, красные=стоп),
стресс-метрика три шкалы (Паузы/Откаты/Хаос)

3. SKILL ATLAS:
Тёмный фон с еле заметной сеткой
Центральный узел gold (класс персонажа)
5 цветных ветвей расходятся, каждая своим цветом секции
Узлы: крупные keystone + малые обычные
Пройденные: залиты цветом + свечение + чекмарк
Заблокированные: серые + пунктирные связи
Узел Вознесения ⚜ — особый, требует 80%+ в двух секциях

4. ПРОФИЛЬ / WEEKLY REPORT:
Hero: персонаж SVG + класс + уровень + XP бар
Weekly AI Report: 4 метрики-карточки + тепловая карта активности по дням
Мини Skill Atlas с dim-пульсацией слабых узлов
Карьерная линия: Junior → Middle → Senior → Staff (горизонтально)
3 карточки AI-рекомендаций с кнопками действий

5. АРЕНА 1v1:
Split layout: два Monaco редактора рядом
Центр сверху: большой таймер с обратным отсчётом
Над редакторами: имя + рейтинг каждого игрока
Статус тестов в реальном времени (зелёный прогресс / красный fail)

Дизайн должен ощущаться как настоящая RPG-игра, а не учебный сервис.
Пользователь — герой, строящий билд. Каждый экран усиливает это ощущение.
```
