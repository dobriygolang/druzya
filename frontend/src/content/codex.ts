...
// Сводки по категориям — counts вычисляются из массива, не дублируются.
// Если в массиве категория не встречается, count = 0 (и фильтр-чип не
// исчезнет — пользователю полезно увидеть, что раздел существует).
export type CodexCategoryWithCount = CodexCategory & { count: number }

export function codexCategoriesWithCounts(): CodexCategoryWithCount[] {
