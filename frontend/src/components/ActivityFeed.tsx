    ...
    >
      <header className="flex items-start justify-between gap-3">
        <div className="flex flex-col gap-1">
          ...
          <h2 className="font-display text-base font-bold leading-tight">
            {summary.last7d > 0
              ? `${summary.last7d} ${pluralActions(summary.last7d)} за 7 дней`
              : 'Журнал пуст'}
          </h2>
        </div>
      ...
      {items.length === 0 ? (
        <p className="text-[13px] italic text-text-muted">
          Пока ничего не залогировано. Каждое занятие — сигнал coach'у: readiness
          подрастёт, daily plan адаптируется. Жми «+ Занятие» когда решишь LeetCode /
          прочтёшь главу / сделаешь mock.
        </p>
      ) : (
              ...
              <li key={a.id} className="flex items-start gap-3 py-2.5">
                ...
                <div className="flex flex-1 flex-col gap-0.5 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 font-mono text-[10px] text-text-muted">
                    ...
                    <span>·</span>
                    <span>{formatAgo(a.occurredAt)}</span>
                  </div>
                </div>
                <button
                  type="button"
      ...
      {items.length > LIMIT_DEFAULT && (
        ...
        >
          {showAll
            ? `Свернуть · показано ${items.length}`
...
function pluralActions(n: number): string {
  // 1 занятие / 2-4 занятия / 5+ занятий
...
function formatAgo(ms: number): string {
  ...
  if (h < 24) return `${h} ${pluralHours(h)} назад`
  const d = Math.round(h / 24)
  if (d <= 6) return `${d} ${pluralDays(d)} назад`
