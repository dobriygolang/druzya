/**
 * druz9 Input + Textarea — web foundation primitive (visual-language v2).
 *
 * Form fields are underline-only per v2 spec. Bordered variant is a legacy
 * fallback (e.g. wrapped select). NEW building block — does NOT migrate any
 * existing input usages.
 *
 * Contract:
 *  - `underline` variant (default):
 *      border-bottom: 1px solid var(--hair-2); padding-bottom: 8px;
 *      background: transparent; outline: none;
 *      focus: border-bottom-color rgb(var(--ink)); border-bottom-width 1.5px
 ...
 *      1.5×24px red signal stripe rendered before the label (v2 "signal stripe").
 ...
 *  - Tokens: var(--hair-2), rgb(var(--ink)), var(--ink-60), var(--red),
...
function buildFieldStyle(
  ...
  // For underline: compensate padding-bottom so the +0.5px border does not shift layout.
...
export const Input = forwardRef<HTMLInputElement, InputProps>(function Input(
  ...
  // Track focus state via class-free trick: render border via inline styles + focus/blur handlers.
  // To keep this a controlled visual element without state, we lean on the field's pseudo-class
  // via a wrapper: we attach focus/blur handlers that mutate inline style of the input element.
  return (
