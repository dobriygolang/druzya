          ...
          {isLoggedIn ? (
            ...
            <>
              <Link
                ...
                className="inline-flex items-center justify-center rounded-md bg-text-primary px-5 py-2.5 text-sm font-medium text-bg hover:bg-text-primary/90"
              >
                Войти, чтобы принять
              </Link>
  ...
  return (
    <div className="min-h-screen bg-bg text-text-primary">
      <div className="mx-auto flex max-w-2xl flex-col gap-6 px-6 py-12 sm:px-8 sm:py-16">
        <Link to="/welcome" className="font-mono text-[12px] tracking-[0.2em] text-text-muted hover:text-text-primary">
          ← druz9
        </Link>
        {children}
      </div>
...
function Stat({ label, value, mono = false, accent = false }: { label: string; value: string; mono?: boolean; accent?: boolean }) {
  return (
    <div className={`rounded-lg border ${accent ? 'border-warn/40 bg-warn/5' : 'border-border bg-surface-2'} px-3 py-2.5`}>
      ...
      <div className={`mt-0.5 text-sm ${mono ? 'font-mono tabular-nums' : ''} ${accent ? 'text-warn' : 'text-text-primary'}`}>
        {value}
      </div>
    </div>
  )
...
function ErrorState({ message }: { message: string }) {
  return (
    <Card className="flex-col gap-2 p-5" interactive={false}>
      <div className="font-mono text-[10px] uppercase tracking-[0.16em] text-danger">
        Ошибка
      </div>
