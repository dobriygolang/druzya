...
export function WarPanel({ cohortId }: { cohortId: string | undefined }) {
  const { data: war, isLoading } = useCohortWarQuery(cohortId)
  if (isLoading) {
    return (
...
export function ActionsPanel({ cohortId, isMine }: { cohortId: string; isMine: boolean }) {
      ...
      <Card className="flex-col gap-3 p-5">
        ...
        {feedback ? (
          ...
          <p className="text-[11px] text-text-muted">
            ...
            Последний участник распускает когорту при выходе.
          </p>
        )}
      </Card>
    )
