...
const t = (key: string, opts?: Record<string, unknown>): string =>
  i18n.t(`pages:readiness.${key}`, opts)
  ...
  switch (goal.kind) {
    case 'top_tier_co': {
      ...
      if (co === 'Google' || co === 'Meta' || co === 'Amazon') {
        return { base: 20, label: t('base.faang', { co }) }
      }
      if (co) {
        return { base: 30, label: t('base.top_tier_named', { co }) }
      }
      return { base: 30, label: t('base.top_tier_generic') }
    }
    case 'ml_offer':
      return { base: 25, label: t('base.ml_offer') }
    case 'any_senior':
      return { base: 40, label: t('base.any_senior') }
    case 'english_target':
      return { base: 35, label: t('base.english') }
    case 'custom':
      return { base: 35, label: t('base.custom') }
  }
}
...
function applyDiagnosticAdjustments(answers: AnswerMap, factors: ReadinessFactor[]): number {
  ...
  switch (answers.experience) {
    case '5_plus':
      delta += 25
      factors.push({ label: t('factor.exp.5_plus'), delta: 25 })
      break
    case '3_5':
      delta += 15
      factors.push({ label: t('factor.exp.3_5'), delta: 15 })
      break
    case '1_3':
      delta += 5
      factors.push({ label: t('factor.exp.1_3'), delta: 5 })
      break
    case '0_1':
  ...
  switch (answers.daily_budget) {
    case '4_plus':
      delta += 10
      factors.push({ label: t('factor.budget.4_plus'), delta: 10 })
      break
    case '2_4':
      delta += 5
      factors.push({ label: t('factor.budget.2_4'), delta: 5 })
      break
    case '0_1':
      delta -= 5
      factors.push({ label: t('factor.budget.0_1'), delta: -5 })
      break
  }
  ...
  switch (answers.status) {
    ...
    case 'refreshing':
      delta += 3
      factors.push({ label: t('factor.status.steady'), delta: 3 })
      break
    case 'between_jobs':
      delta -= 5
      factors.push({ label: t('factor.status.between'), delta: -5 })
      break
  }
  ...
  if (
    ...
    delta -= 10
    factors.push({
      label: t('factor.weak.sysdesign_high'),
      delta: -10,
    })
  }
  ...
  if (answers.weakest === 'algos' && answers.target_co === 'big_tech') {
    delta -= 5
    factors.push({
      label: t('factor.weak.algo_faang'),
      delta: -5,
    })
  }
...
function applyTimeAdjustments(
  ...
  if (daysToTarget < 30) {
    delta -= 10
    factors.push({ label: t('factor.time.lt_month', { date: goal.targetDate }), delta: -10 })
  } else if (daysToTarget < 90) {
    delta -= 5
    factors.push({ label: t('factor.time.lt_quarter', { date: goal.targetDate }), delta: -5 })
  } else if (daysToTarget > 365) {
    delta += 5
    factors.push({ label: t('factor.time.gt_year'), delta: 5 })
  }
