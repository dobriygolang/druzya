...
export const matchesHandlers = [
    http.get(`${base}/matches/history`, () => HttpResponse.json({
        total_wins: 284,
        total_losses: 176,
        avg_lp: 2.4,
        matches,
    ...
    http.get(`${base}/arena/matches/my`, ({ request }) => {
        ...
        const filtered = allItems.filter((i) => (!modeFilter || i.mode === modeFilter) && (!sectionFilter || i.section === sectionFilter));
    ...
    http.get(`${base}/matches/:id/end`, ({ params }) => HttpResponse.json({
        id: params.id,
        result: 'W',
