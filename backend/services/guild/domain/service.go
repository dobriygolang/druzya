...
// SideForGuild returns which side of the war the guild ID belongs to.
// Returns false when the guild is not a participant.
func SideForGuild(w War, guildID uuid.UUID) (Side, bool) {
	switch guildID {
	case w.GuildAID:
		return SideA, true
	case w.GuildBID:
...
// DetermineWinner walks the five lines and returns the guild id that won the
