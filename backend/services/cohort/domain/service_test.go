...
func TestSideForCohort(t *testing.T) {
	t.Parallel()
	w := mkWar(nil, nil)
	s, ok := SideForCohort(w, w.CohortAID)
	if !ok || s != SideA {
		t.Fatalf("CohortA should map to SideA, got %q ok=%v", s, ok)
	}
	s, ok = SideForCohort(w, w.CohortBID)
	if !ok || s != SideB {
		t.Fatalf("CohortB should map to SideB, got %q ok=%v", s, ok)
	}
	if _, ok := SideForCohort(w, uuid.New()); ok {
...
func TestCanContribute_ActiveAndSection(t *testing.T) {
	...
	m := Member{UserID: uuid.New(), CohortID: w.CohortAID, AssignedSection: &algo}
	...
	mFree := Member{UserID: uuid.New(), CohortID: w.CohortAID}
...
func TestDetermineWinner_MajorityA(t *testing.T) {
	...
	if got == nil || *got != w.CohortAID {
		t.Fatalf("expected cohort A, got %v", got)
	}
}
...
func TestDetermineWinner_MajorityB(t *testing.T) {
	...
	got := DetermineWinner(w)
	if got == nil || *got != w.CohortBID {
		t.Fatalf("expected cohort B, got %v", got)
	}
}
...
func TestDetermineWinner_EmptyScoresIsDraw(t *testing.T) {
	t.Parallel()
	// Both cohorts at zero everywhere → draw.
