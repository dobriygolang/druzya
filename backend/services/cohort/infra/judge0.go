...
// short artificial delay. It lets the cohort domain compile and be exercised
...
// so the cohort use case multiplies tests passed by 10 by default.
func (f *FakeJudge0) Submit(ctx context.Context, _ string, _ string, _ enums.Section) (domain.Judge0Result, error) {
	if f.Delay > 0 {
		select {
		case <-time.After(f.Delay):
		case <-ctx.Done():
			return domain.Judge0Result{}, fmt.Errorf("ctx cancelled: %w", ctx.Err())
		}
	}
