...
// выгоден. Перенесено дословно из
// cmd/monolith/services/admin/llmchain_config.go.
package infra
...
// LLMChainConfig is the Postgres adapter.
type LLMChainConfig struct {
	pool *pgxpool.Pool
}

// NewLLMChainConfig wraps a pool.
func NewLLMChainConfig(pool *pgxpool.Pool) *LLMChainConfig {
	return &LLMChainConfig{pool: pool}
}

// Load читает singleton row и парсит JSONB поля в typed llmchain-структуры.
// ErrNoRows (свежая БД без INSERT через миграцию) → пустой RuntimeConfig.
func (s *LLMChainConfig) Load(ctx context.Context) (*llmchain.RuntimeConfig, error) {
	const q = `
...
func (s *LLMChainConfig) Save(ctx context.Context, cfg *llmchain.RuntimeConfig, expectedVersion int64) error {
