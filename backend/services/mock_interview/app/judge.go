...
type JudgeCanvasInput struct {
	...
	// Locale — user's preferred response language ("ru"/"en"); empty → "ru".
	Locale string
}
...
type JudgeInput struct {
	...
	// Locale is the user's preferred response language ("ru" or "en");
	// empty defaults to "ru". Caller (orchestrator) resolves via
	// userlocale.Reader once per attempt. Used to inject a language
	// directive as the first system message in pass1 + pass2 LLM calls.
	Locale string
}
...
Схема ответа:
{
  ...
  "feedback": "<2-4 предложения in the user's preferred language: strengths, what to improve>"
}
...
func (j *LLMJudge) pass1WaterScore(ctx context.Context, in JudgeInput) (float64, error) {
	...
	resp, err := j.chain.Chat(ctx, llmchain.Request{
		...
		Messages: []llmchain.Message{
			{Role: llmchain.RoleSystem, Content: userlocale.LanguageDirective(in.Locale)},
...
ВАЖНО про диаграмму:
- Оценивай АРХИТЕКТУРНОЕ МЫШЛЕНИЕ, а не аккуратность отрисовки. Кандидат рисует на онлайн-доске под таймером — кривые линии, схематичные прямоугольники и от руки подписанные стрелки — это норма.
- НИКОГДА не снижай score за: «нарисовано схематично», «не очень аккуратно», «прямоугольники неровные», отсутствие легенды/цветовой схемы, прерывистые стрелки. Это не оценивается.
- Идентифицируй компоненты по подписям и расположению, а не по визуальной полировке. Если подпись «Cassandra» — это Cassandra, даже если рисунок шерстяной.
- Если на диаграмме совсем нет элементов или элементы пустые/нулевого размера — это единственный случай когда визуал важен: верни score=0, missing_points=["диаграмма пустая"].
