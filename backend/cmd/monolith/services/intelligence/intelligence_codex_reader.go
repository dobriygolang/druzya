...
// ── intelCodexReader: curated Codex article suggestions ──

type intelCodexReader struct{ pool *pgxpool.Pool }

type codexCandidate struct {
	article intelDomain.CodexArticleSuggestion
	text    string
...
func (r *intelCodexReader) SuggestArticles(ctx context.Context, _ uuid.UUID, topics []string, limit int) ([]intelDomain.CodexArticleSuggestion, error) {
	if limit <= 0 || limit > 12 {
		limit = 6
	}
	...
	rows, err := r.pool.Query(ctx,
		`SELECT a.slug,
		  ...
		  LIMIT 200`)
	if err != nil {
		return nil, fmt.Errorf("intelligence.intelCodexReader: %w", err)
	}
	defer rows.Close()
	candidates := make([]codexCandidate, 0, 32)
	order := 0
	for rows.Next() {
		var a intelDomain.CodexArticleSuggestion
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("intelligence.intelCodexReader: scan: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("intelligence.intelCodexReader rows: %w", err)
	}
	...
	out := make([]intelDomain.CodexArticleSuggestion, 0, len(candidates))
...
func codexSearchText(a intelDomain.CodexArticleSuggestion, categoryLabel, categoryDescription string) string {
