...
-- 00022_qa_devops_atlas_seed_data.sql
--
-- Seed данные для QA / DevOps треков. Отделены от 00019 потому что
-- Postgres запрещает использовать только что созданную ENUM-метку
-- ('devops' из 00019) в той же транзакции, где она появилась.
-- Раздельная миграция = новая транзакция = безопасный INSERT.
--
