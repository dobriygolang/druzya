...
Технически: Electron + Vite + React 18, Connect-RPC к Go-бэкенду, ~13 страниц, AI-фичи через `llmchain` (Groq → Cerebras → Mistral → OpenRouter). English Reading-loop с Leitner SRS поверх backend `services/hone/`, vocab queue + summary grader + writing grader через free-tier LLM.
