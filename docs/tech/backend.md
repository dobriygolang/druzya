...
tracks           transcription   tutor           whiteboard_rooms
```
...
| `podcast` | Podcasts CMS + плеер (Hone surface переезжает в web — см. Phase G roadmap) |
...
Миграции в `backend/migrations/`. Consolidated baseline `00001_baseline.sql` (~3.1k строк, финальная схема + Phase 1-4 / Wave 0-6 inline) + ~80 patch-миграций сверху. Текущий последний номер — `00095` (после 2026-05-12 marathon: F6 deprecate, RPG cleanup, F2 user_primary_goals, F10 cue_sessions, drop use_for_arena, subscription BYOK, google_calendar, drop_peer_collab, stage_templates, tutor_mode_paths, user_milestones, stripe_subscriptions, goal_presets). Полный обзор по группам — [backend/migrations/README.md](../../backend/migrations/README.md).
