...
import type { IntelligenceClient } from '../api/intelligence';
...
export interface ManagerDeps {
  ...
  /** F10 (Phase D 2026-05-12) — optional ingestion к backend intelligence.
   * Best-effort: failure не блокирует note save / hone import. */
      ...
      try {
        ...
        if (analysis.status === 'ready' || analysis.status === 'failed') {
          ...
          if (analysis.status === 'ready') {
            ...
            // F10 cross-product moat — POST к backend intelligence service
            // так что coach memory + web /today CueSessionsSection видят
            // новую interview session. Best-effort: anonymous-on-Cue или
            // backend unreachable → silent skip (note locally сохранён).
            if (deps.intelligence) {
              void deps.intelligence
                .ingestInterviewSession(analysis)
                .catch(() => {
                  /* silent — local note + Hone import already secured user data */
                });
            }
          }
          return;
