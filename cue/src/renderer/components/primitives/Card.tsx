/**
 * Cue Card — stealth tray-copilot foundation primitive (visual-language v2).
 *
 * NEW building block. Mirrors web/Hone Card API; Cue palette uses `--d9-*` tokens
 * (--d9-hairline / --d9-hairline-b / --d9-accent / --d9-ink). HUD compact density
 * recommended: pad='sm' is the typical default for tray surfaces.
 *
 * Contract (visual-language v2):
 *  - 4 elevations:
 *      e0 — transparent (no bg, no border)
 *      e1 — bg rgba(255,255,255,0.02) + 1px var(--d9-hairline) border (DEFAULT)
 *      e2 — bg #0a0a0a + 1px var(--d9-hairline-b) + soft drop shadow
 *      e3 — bg #0a0a0a + 1px var(--d9-hairline-b) + deep window-shadow
 *  - `lift`: hover translateY(-1px) + border opacity 8% → 14%
 *  - `signal`:
 *      'active' / 'live' → 1.5×24px var(--d9-accent) left stripe;
 *         'live' adds `.red-pulse` breathing
 *      'error' → 1.5px var(--d9-accent) border decays back to hairline-b over 1200ms
  ...
  switch (elevation) {
    ...
    case 'e1':
      return {
        ...
        border: `1px solid ${hovered && lift ? 'var(--d9-hairline-b)' : 'var(--d9-hairline)'}`,
        boxShadow: undefined,
      };
    case 'e2':
      return {
...
export const Card = forwardRef<HTMLElement, CardProps>(function Card(
  ...
  const baseStyle: CSSProperties = {
    ...
    border: isError ? `1.5px solid var(--d9-accent)` : elev.border,
