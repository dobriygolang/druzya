...
export function ToastScreen() {
  ...
  // B/W + red rule: error → canonical red signal, warn → ink-ramp 75%
  // (urgency through intensity, not hue), info → full white.
  const accent =
    params.kind === 'error'
      ? 'var(--d9-accent)'
      : params.kind === 'warn'
  ...
  return (
    <div
      className="d9-root"
      onClick={() => {
        void window.druz9.toast.dismiss();
      }}
      role={params.kind === 'error' ? 'alert' : 'status'}
      aria-live={params.kind === 'error' ? 'assertive' : 'polite'}
      aria-atomic="true"
      style={{
        width: '100%',
        height: '100%',
      ...
      >
        ...
        <div
          style={{
            width: 3,
            background: accent,
            // Glow only for error (canonical red signal). Warn/info — no glow,
            // intensity comes from the bar opacity alone.
            boxShadow: params.kind === 'error' ? `0 0 12px ${accent}` : 'none',
          }}
        />
        {/* Icon + message body */}
        <div
        ...
        >
          <div
            style={{
              ...
              fontWeight: 700,
              fontSize: 13,
              boxShadow: params.kind === 'error' ? `0 0 10px ${accent}` : 'none',
            }}
          >
            {iconChar}
          </div>
