...
function StepDots({ current }: { current: Step }) {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', gap: 'var(--pad-inline)', padding: '24px 0 0' }}>
      {order.map((_, i) => (
        <div
          key={i}
          style={{
            ...
            height: 3,
            borderRadius: 2,
            // v2: progress bar filled with white ink (was red accent which
            // confused "active step" with "live signal" — red is now reserved
            // for canonical live/danger states only).
            background: i <= idx ? 'var(--d9-ink)' : 'var(--d9-hairline-b)',
  ...
  return (
    <div style={{ textAlign: 'center', maxWidth: 440 }}>
      ...
      <p style={{ fontSize: 14, color: 'var(--d9-ink-dim)', lineHeight: 1.5, margin: '0 0 28px' }}>
        Тихий AI-companion для подготовки к interview и работы в open-plan офисе.
        Live-транскрипт встреч, скриншот → ответ, English Polish — всё в floating
        окне которое уважает твою privacy (visibility toggle в Settings).
      </p>
      ...
      {(perms?.screenRecording !== 'granted' || perms?.accessibility !== 'granted') && (
        <div
          style={{
            ...
            gap: 12,
            padding: '10px 14px',
            borderRadius: 'var(--radius-inner, 9px)',
        ...
        >
          {/* v2 signature: red signal stripe для system-info notice */}
          <span aria-hidden="true" style={{ display: 'inline-block', width: 1.5, minHeight: 32, background: 'var(--d9-accent)', marginTop: 4, flex: '0 0 auto' }} />
          <span>
            <b style={{ color: 'var(--d9-ink)' }}>Если переключатель уже включён, а доступа «нет»</b>
            {' '}— macOS кэширует статус до рестарта процесса. Включи тоггл в Системных настройках,
            затем нажми «Рестарт» — приложение перезапустится и подтянет свежее состояние.
          </span>
        </div>
      )}
    ...
    >
      <div
        style={{
          ...
          // v2: granted → white-filled square + black check (canonical success
          // = inverted ink). Ungranted → hairline + red signal-tint icon.
          background: granted ? 'var(--d9-ink)' : 'transparent',
          border: granted ? 0 : '1px solid var(--d9-hairline-b)',
          color: granted ? 'var(--d9-obsidian)' : 'var(--d9-accent)',
          flexShrink: 0,
        }}
      >
  ...
  if (state.kind === 'waiting') {
    return (
      <div style={{ textAlign: 'center', maxWidth: 460 }}>
        ...
        <div style={{ marginTop: 18, display: 'flex', justifyContent: 'center', gap: 'var(--pad-inline)' }}>
          <Button
            variant="secondary"
            size="sm"
    ...
    <div style={{ textAlign: 'center', maxWidth: 440 }}>
      <div
        style={{
          ...
          height: 56,
          borderRadius: 16,
          // v2: success ring — white-filled circle + black check (was green
          // tinted bg + green check).
      ...
      <h2 style={{ fontSize: 22, margin: '16px 0 8px', letterSpacing: '-0.018em', color: 'var(--d9-ink)' }}>
        Всё готово
      </h2>
      <p style={{ fontSize: 13, color: 'var(--d9-ink-dim)', lineHeight: 1.55, margin: '0 0 18px' }}>
