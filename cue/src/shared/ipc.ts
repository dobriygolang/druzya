...
export const invokeChannels = {
  ...
  /** Persist a single override (accelerator='' clears it). Main writes
   *  userData/hotkeys.json + re-registers globalShortcut so the new chord
   *  fires immediately. Returns the updated override map for renderer
   *  stores. */
  hotkeysSetOverride: 'hotkeys:set-override',
  /** Read the persisted override map without re-applying bindings. Used
   *  on settings mount to hydrate the renderer's store from main. */
  hotkeysListOverrides: 'hotkeys:list-overrides',
...
export interface Druz9API {
  ...
  hotkeys: {
    ...
    /** Persist a single override (accelerator='' clears it). Main writes
     *  userData/hotkeys.json + re-registers globalShortcut. Returns the
     *  updated override map. */
    setOverride: (
      action: HotkeyAction,
      accelerator: string,
    ) => Promise<Partial<Record<HotkeyAction, string>>>;
    /** Read the persisted override map (no side-effects). */
    listOverrides: () => Promise<Partial<Record<HotkeyAction, string>>>;
  };
  windows: {
   ...
   * (cue/native/audio-mac/build.sh). On Windows/Linux the feature
