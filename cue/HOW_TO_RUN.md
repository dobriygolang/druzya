...
make cue-install  # npm install в cue/
```
...
# 2. Cue (в отдельном терминале)
# Если бек локально:
make cue-dev

# Если бек на твоём сервере:
DRUZ9_API_BASE_URL=https://api.твойсервер.tld make cue-dev
```
...
```bash
DRUZ9_API_BASE_URL=https://api.твойсервер.tld make cue-build
```

Готовый `.dmg` — в `cue/dist/Druz9 Copilot-0.1.0-arm64.dmg`
...
- **В prod-билде (`.dmg`):** тот же env var во время `make cue-build`.
