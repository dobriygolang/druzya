...
//
// Run with:
//   make cue-build-masquerade
//
// The main `.dmg` (real Druz9 branding) is still built by the normal
// `make cue-build` target — this script is purely for the disguise
...
const cueDir = resolve(__dirname, '..');
...
function run() {
  ...
  for (const preset of filtered) {
    ...
    const outDir = join(cueDir, 'dist', 'masquerade', preset.id);
  ...
  console.log('Outputs in cue/dist/masquerade/<preset>/*.dmg');
  console.log(
...
function writePresetConfig(preset) {
  ...
  const outPath = join(cueDir, `.masquerade-${preset.id}.yml`);
...
function readBaseConfig() {
  ...
  const raw = readFileSync(join(cueDir, 'electron-builder.yml'), 'utf-8');
