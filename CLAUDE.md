...
**Phase K Wave 16 shipped 2026-05-14 (100% coverage)** — i18n унификация (web + Hone + Cue + backend LLM). **Locale scope: только `ru` и `en`. Legacy KZ/UA fallback chain полностью удалён.** Single source of truth: `users.locale` ('ru'/'en'). Backend: shared `backend/shared/pkg/userlocale/` (Reader + LRU + `LanguageDirective` + `ResponseLanguagePolicy`). LLM-сервисы (copilot Suggest/Analyze/Chat, intelligence brief+critique, mock_interview judge Pass1/Pass2/Canvas) injектят language directive слотом 0. ai_mock postgres.go грузит `users.locale` через `GetUserSubscriptionAndLocale` SQL. Frontend SettingsPage + AppShell `onLang` пишут backend через `useUpdateSettingsMutation({locale})` — без этого LLM игнорировал UI switch. Shared package `shared/i18n/` (root, alias `@d9-i18n`) — flat type-safe dict для Hone+Cue. Storage key унифицирован на `'druz9.locale'`. Lingua/english_hr mock — `PolicyForceEnglish` (English by design). Migration `00125_users_locale_check.sql` (CHECK ru/en). **Final coverage stats**: Frontend = 0 ESLint Cyrillic warnings (down from ~2805), 18 namespaces in parity, 5 новых (auth/toasts/lingua/admin/tutor) + activity/atlas/invite/wave14 (8 новых total). Hone = ~55 файлов swept, 283 `hone.*` keys в shared/i18n. Cue = ~30 файлов swept, 520 `cue.*` keys в shared/i18n. **Все 3 tsc — exit 0**. Backend tests все pass. ESLint rule `d9-i18n/no-cyrillic-literals` flipped с `'warn'` на `'error'` — регрессии теперь блокируют merge. KZ/UA legacy полностью вычищен (wave10 _note, stripeCheckout.ts `kk`/`be`, hone UpgradeModal `kk`/`be`, AppShell comment). Tooling: `tools/eslint-no-cyrillic-literals.cjs` + `make check-i18n-parity` / `check-i18n-unused` / `check-i18n`.