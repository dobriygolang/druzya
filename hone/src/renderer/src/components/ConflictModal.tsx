...
export function emitConflict(detail: ConflictPayload): void {
  window.dispatchEvent(new CustomEvent('hone:conflict', { detail }));
}
...
export function useConflictListener(): void {
  ...
  useEffect(() => {
    ...
    window.addEventListener('hone:conflict', h as EventListener);
    return () => window.removeEventListener('hone:conflict', h as EventListener);
  }, [open]);
}
...
export function ConflictModal(): JSX.Element | null {
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <header style={{ padding: '16px 20px 12px', borderBottom: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          <div>
            <div
        ...
        {!mergeMode ? (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, flex: 1, minHeight: 0, background: 'rgba(255,255,255,0.06)' }}>
            <Side
              label="Local version"
        ...
        {/* Footer: actions */}
        <footer style={{ padding: '12px 20px', display: 'flex', justifyContent: 'flex-end', gap: 8, borderTop: '1px solid rgba(255,255,255,0.08)', flexWrap: 'wrap' }}>
          {!mergeMode ? (
            <>
              <button
