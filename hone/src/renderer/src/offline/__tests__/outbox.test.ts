...
//
// Tested invariants (см H7 / Phase J task spec):
...
//   • 409 conflict → nonRetryable → dead-letter (текущий behavior; see TODOs)
...
describe('outbox — 409 conflict (current behavior)', () => {
  ...
  //
  // TODO(H7-followup): «409 = client marks as success» был задуман в product
  // spec'е, но текущий code path делает dead-letter. Если backend начнёт
  // когда-нибудь reuse'ить 409 для idempotency-replay (тип «server already
  // has it» вместо version mismatch), нужно будет добавить kind в
  // rpcError'е чтобы выделять success-aliased 409 от conflict-409.
