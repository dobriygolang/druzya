...
import { getUserSettings } from '../api/hone';
...
async function hadEnglishFootprint(): Promise<boolean> {
  ...
  try {
    const raw = localStorage.getItem('hone:profile:v2');
    if (raw) {
  ...
  // Backend probe — only if local heuristics didn't fire.
  try {
    const settings = await getUserSettings();
    if (settings.englishActive) return true;
  } catch {
    /* offline or 401 — treat as no signal; modal will surface next time
